  ...
  function initETSLocationFinder() {
    ...
    function selectLocation(locationId, options) {
      ...
      // Move card to top if requested (for pin click)
      if (opts.bringToTop) {
        bringCardToTop(loc);
      }
      ...
      // Scroll card into view (only when explicitly asked)
    ...
    function createMarkersAndWireCards() {
      ...
      locations.forEach(loc => {
        ...
        // 🔹 PIN CLICK: move card to top, no scroll
      ...
      // 🔹 CARD CLICK: flyTo + popup, no reordering
      locations.forEach(loc => {
        loc.cardEl.style.cursor = 'pointer';
        loc.cardEl.addEventListener('click', function () {
          selectLocation(loc.id, {
            flyTo: true,
            openPopup: true,
            scrollToCard: false,
    ...
    async function calculateAndApplyDistances(originLatLng) {
      ...
      try {
        ...
        // Zoom between user location & nearest
    ...
    // Ask for location ON LOAD (distance + sorting)
    if ('geolocation' in navigator) {
